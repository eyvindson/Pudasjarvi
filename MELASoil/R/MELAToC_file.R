#Reads in all MELA input from one file (if NewWay = TRUE in MELAToC_definitions.R)
#Returns input data in data frames MelaStock, MelaNatMortality and MelaWaste,
#and areas in data frame Areas and years of MELA input in vector MelaYears


#First, definition of helper functions
...
#gregexpr searches for matches to argument ";"
...
l <- gsub(",",".",lines[alku[1]])    #replace , -> . and return
p <- gregexpr(";",l)[[1]]
sfUpland <- as.numeric(substr(l,p[1]+1,p[2]-1))
l <- gsub(",",".",lines[alku[1]+2])    #replace , -> . and return
p <- gregexpr(";",l)[[1]]
sfUndrained <- as.numeric(substr(l,p[1]+1,p[2]-1))
l <- gsub(",",".",lines[alku[1]+3])    #replace , -> . and return
p <- gregexpr(";",l)[[1]]
sfDrained <- as.numeric(substr(l,p[1]+1,p[2]-1))
#
l <- gsub(",",".",lines[alku[2]])    #replace , -> . and return
p <- gregexpr(";",l)[[1]]
nfUpland <- as.numeric(substr(l,p[1]+1,p[2]-1))
l <- gsub(",",".",lines[alku[2]+2])    #replace , -> . and return
p <- gregexpr(";",l)[[1]]
nfUndrained <- as.numeric(substr(l,p[1]+1,p[2]-1))
l <- gsub(",",".",lines[alku[2]+3])    #replace , -> . and return
p <- gregexpr(";",l)[[1]]
...
# There are 9 tree - site combinations for Southern and Northern Finland
# Mortality is given as m3/year between two points of time. There are thus one
# value less than for stock 
...
# Mortality is given as t DM / year between two points of time. There are thus one
# value less than for stock
...
for(i in 0:44) {
	wb <- 1:(nYears-1)
	l <- gsub(",",".",lines[alku[1]+wasteRow+i])    #replace , -> . and return
	soil <- as.integer(substr(l,1,1))
	species <- as.integer(substr(l,3,3))
...
for(i in 0:44) {
	wb <- 1:(nYears-1)
	l <- gsub(",",".",lines[alku[2]+wasteRow+i])    #replace , -> . and return
	soil <- as.integer(substr(l,1,1))
	species <- as.integer(substr(l,3,3))
	part <- as.integer(substr(l,5,5))
	ind <- gregexpr(";",l)[[1]]
