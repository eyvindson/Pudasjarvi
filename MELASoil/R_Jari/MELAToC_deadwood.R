...
dt.nf <- DeltaTNorth
#LCF
#    pyrs<-c(2006,2010,2012,2018,2025,2035,2050)
#    padd.sf<-c(0,0.3,0.7,1.4,1.45,3.1,6.1)/100.0   #per cent -> share
#    padd.nf<-c(0,0.45,1,1.7,1.75,3.6,7)/100.0   #per cent -> share

#CliPan 2018
#pyrs <- c(2000,2010,2020,2030,2040,2050,2060,2070)
#padd.sf <- c(0.000, 32.856, 33.414, 23.119, 61.123, 93.117, 54.531, 27.209)/100  #SFin
#padd.sf <- c(0.000, 5.327, 21.099, 13.446, 50.048, 89.575, 49.727, 35.380)  #CFin
#padd.nf <- c(0.000, -8.007, 29.253, -7.708, 44.469, 80.870, 47.391, 35.343)/100

#    dpr.sf<-approxfun(pyrs,padd.sf,rule=2)
#    dpr.nf<-approxfun(pyrs,padd.nf,rule=2)

#tassa LFC muutos
#    dgpr.sf<-approxfun(pyrs,padd.sf-dpr.sf(2010),rule=2)
#    dpr.nf<-approxfun(pyrs,padd.nf-dpr.nf(2010),rule=2)
#CliPan 2018
    #dpr.sf<-approxfun(pyrs,padd.sf-dpr.sf(2011),rule=2)
    #dpr.nf<-approxfun(pyrs,padd.nf-dpr.nf(2011),rule=2)

dpr.sf <- PrecChgSouth
dpr.nf <- PrecChgNorth

#if(CC) cc = 1.0 else cc = 0.0
if(YassoCC) cc = 1.0 else cc = 0.0
