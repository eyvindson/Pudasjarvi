...
if __name__ == '__main__':
    ...
    parser.add_argument('--partition1', help='partition size Lower limit', type=str)
    parser.add_argument('--partition2', help='partition size upper limit', type=str)
    args = parser.parse_args()
    
    #values = glob.glob('/proj/project_2000611/SompaMaps/soil/SpaFHyPeat_Kyle/SpaFHyPeat/'+args.folder+'/parameters/Ncoord*.dat')
    

    #values = [t.replace('/proj/project_2000611/SompaMaps/soil/SpaFHyPeat_Kyle/SpaFHyPeat/'+args.folder+'/parameters/Ncoord','').replace(".dat","") for t in values]



    #import os
    #values = []
    #for fyle in glob.glob('/proj/project_2000611/SompaMaps/soil/SpaFHyPeat_Kyle/SpaFHyPeat/'+args.folder+'/parameters/Ncoord*.dat'):
    #    if os.stat(fyle).st_size >= int(args.partition1):
    #        if os.stat(fyle).st_size < int(args.partition2):
    #            values = values + [fyle]
