...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 14:01:35 2019
...
def change_input(file_list,fpath,N_partitions):
    for file in file_list:
        ...
        division = 1
        f2 = file
        if file == "ditch_depth_mean.dat":
            file = "ditch_depth.dat"
        if file == "UPDATED_Fertility.dat":
            file = "soil_id.dat"
        ...
        for N_part in [n for n in range(0,N_partitions)]:
            ...
            for k in [i for i in range(start,R_part[N_part])]:
                if k <= (len(contents)-6):
                    if f2 == "ditch_depth.dat":
                        g.write(str(float(contents[k+6])/100)+'\n')
                    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Region', help='which region', type=str)
        
    args = parser.parse_args()
    file_list = ['Ncoord.dat','Ecoord.dat','LAI_spruce.dat','LAI_pine.dat','LAI_decid.dat','hc.dat','soil_id.dat','forcing_id.dat','ditch_spacing.dat','ditch_depth.dat','cf.dat']
    os.chdir("/scratch/project_2000611/SOMPASOILS/SompaMaps/soil/SpaFHyPeat_Kyle/SpaFHyPeat/")
    #file_list = ['Ncoord.dat','Ecoord.dat','LAI_spruce.dat','LAI_pine.dat','LAI_decid.dat','hc.dat','soil_id.dat','forcing_id.dat','ditch_spacing.dat','ditch_depth.dat','cf.dat']
    if args.Region in ["amaa"]:
        fpath = '/scratch/project_2000611/SOMPASOILS/SompaMaps/soil/SpaFHyPeat_Kyle/SpaFHyPeat/'+args.Region+'/parameters/'
        N_partitions = 10
        change_input(file_list,fpath,N_partitions)
        print("Success")
    elif args.Region in ['pkarjala','lappi','itasuomi','lansisuomi',"Test_case"]:
        N_partitions = 100
        fpath = '/scratch/project_2000611/SOMPASOILS/SompaMaps/soil/SpaFHyPeat_Kyle/SpaFHyPeat/'+args.Region+'/parameters/'
        change_input(file_list,fpath,N_partitions)
        print("Success")
