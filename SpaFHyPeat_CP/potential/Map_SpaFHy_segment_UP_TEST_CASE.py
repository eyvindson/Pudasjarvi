...
def N2O_CH4_C2O_EMISSIONS(GTW,peat_loc,region,duplicate_part,file_start,DD,UPFERT):
    ...
    # if UPFERT == False:
        # reg = "pkarjala"
        # kasvu = np.loadtxt(path + reg + "/segmaj1.kasvupaikka_vmi1x_1216.hits")
        # np.savetxt(path + region +"/"+reg+"_segmaj1.kasvupaikka_vmi1x_1216.hits",kasvu)
        # kasvu = pd.read_csv(path + reg +"/"+reg+"_segmaj1.kasvupaikka_vmi1x_1216.hits",header = None, sep = " ")
    # else:
    ...
    # pixels = np.loadtxt(path + reg + "/segmaj1.pseudotype.hits")
    # np.savetxt(path + region + "/"+region+"_segmaj1.pseudotype.hits",pixels)
    # pixels = pd.read_csv(path + reg + "/"+reg+"_segmaj1.pseudotype.hits",header = None, sep = " ")
    # paa = np.loadtxt(path + reg + "/segmaj1.paatyyppi_vmi1x_1216.hits")
    # np.savetxt(path + region + "/"+region+"_segmaj1.paatyyppi_vmi1x_1216.hits",paa)
    # paa = pd.read_csv(path + reg + "/"+reg+"_segmaj1.paatyyppi_vmi1x_1216.hits",header = None,sep=" ")
    ...
    df9=pd.DataFrame(data=East)
    df10=pd.DataFrame(data=North)
    df12 = pd.concat([df9,df10],axis = 1,sort = False)
    df12.columns = ["E","N"]
    df12 = df12.diff()
    df12.loc[0] = 0
    df12.loc[df12['N'] > 0, 'N'] = 1
    df12.loc[df12['N'] < 0, 'N'] = 1
    df12.loc[df12['E'] > 0, 'E'] = 1
    df12.loc[df12['E'] < 0, 'E'] = 1
    df12['sum'] = df12.loc[:,"N"]+df12.loc[:,"E"]
    df12.loc[df12['sum'] > 0, 'sum'] = 1
    df12['cumsum'] = df12['sum'].cumsum()
    ...
    t_North = df10
    t_North.columns = ['Segment']
    result = pd.merge(df12, t_KASVU, left_on='cumsum', right_index=True,how='left', sort=False)
    t_KASVU = pd.DataFrame(result["Segment"])
    t_KASVU_N2O = t_KASVU.replace(1,20).replace(2,20).replace(3,25).replace(4,29).replace(5,39).replace(6,48).replace(7,48).replace(8,48).replace(9,48).replace(10,48)
    Nut_rich = t_KASVU.replace(1,1).replace(2,1).replace(3,1).replace(4,0).replace(5,0).replace(6,0).replace(7,0).replace(8,0).replace(9,0).replace(10,0)
    Nut_poor = Nut_rich*-1+1
    CH4_y0 =  -0.378
    ...
    df1.columns = ["segment"]
    t_PIXELS.columns = ['Pixels']
    result = pd.merge(result, df1, left_on='cumsum', right_index=True,how='left', sort=False)
    result = pd.merge(result, t_PIXELS, left_on='cumsum', right_index=True,how='left', sort=False)    
    EM_data = {"LAI_DEC":GTW['DEC_LAI'], "LAI_CON":GTW['CON_LAI'],"Pixels":t_PIXELS['Pixels'],"Depth":t_Depth['Segment'],"Ditch":t_Drain['Segment'],"Canopy_height":t_Canopy_height['Segment'],"Canopy_fraction":t_Canopy_fraction['Segment'],"East":t_East['Segment'],"North":t_North['Segment'],"Air_Temp_Mean_ALL":GTW['Air_Temp'],"Precip_Mean_ALL":GTW['Precip'],"Air_Temp_Mean_Last_10":GTW['Air_Temp_10'],"Precip_Mean_Last_10":GTW['Precip_10'],"Soil_type":t_KASVU['Segment']}
    k = 0
    for i in key:
        EM_data_1 = {"N2O"+str(i[8:]):N2O[k],"CH4"+str(i[8:]): CH4_flux[k],"CO2"+str(i[8:]): NetSoil_CO2[k],"GWT"+str(i[8:]): GTW[i]}
        EM_data.update(EM_data_1)
...
if __name__ == '__main__':
    ...
    key = N2O_CH4_C2O_EMISSIONS(DATA1,peat_loc,args.folder,duplicate_part,file_start,D2,False)
