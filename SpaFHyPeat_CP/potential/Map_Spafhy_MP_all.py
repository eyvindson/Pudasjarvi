...
if __name__ == '__main__':
    ...
    pool = mp.Pool(40)
    if args.folder == "amaa":
        DATA =  pool.map(Mapping,[[args.folder,i] for i in glob.glob(args.folder+"*.nc")])
    else:
        DATA =  pool.map(Mapping,[[args.folder,i] for i in glob.glob(args.folder+"*.nc")])
        #DATA =  pool.map(Mapping,[[args.folder,i] for i in range(0,100)])
    DATA1 = DATA[0]
    ...
    input_data = open("/scratch/project_2000611/SOMPASOILS/SompaMaps/soil/SpaFHyPeat_Kyle/SpaFHyPeat/results/"+args.folder+".pkl","rb")
    DATA1 = pickle.load(input_data)

    DATA2 = DATA1[DATA1['z']>-1]
    ax1 = DATA2.plot.scatter(x= "x", y = "y", c = "z",figsize = (20,20),colormap='viridis')
