...
def parameters():
    ...
    pgen = {'description': 'testcase',  # description written in result file
            ...
            'gis_folder': str(pathlib.Path(r'amaa/parameters/')),
            'forcing_file': str(pathlib.Path(r'amaa/forcing/Weather_id_[forcing_id].csv')),
            #'gis_folder': r'amaa/parameters',
            #'forcing_file': r'amaa/forcing/Weather_id_[forcing_id].csv',
            'forcing_id': 0,  # used if spatial_forcing == False
            'ncf_file': r'amaa.nc',
            'results_folder': r'amaa/results/',
