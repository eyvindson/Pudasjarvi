...
# Make a DataArray with the number of days in each month, size = len(time)            
month_length = xarray.DataArray(re_calculate_season.get_dpm(ds['soil_ground_water_level'].date.to_index(), calendar='noleap'),
                            coords=[ds['soil_ground_water_level'].date], name='month_length')
incl_months = [5,6,7,8,9,10]
# Calculate the weights by grouping by 'time.season'.
# Conversion to float type ('astype(float)') only necessary for Python 2.x
weights = month_length.groupby('date.month') / month_length.astype(float).groupby('date.month').sum()
weights = weights.month.isin([5,6,7,8,9,10])*weights

ds= ds.assign(mean_summer_sgwl = (ds['soil_ground_water_level'] * weights).groupby('date.month').sum('date'))

t_lon, t_lat = [round(elm, 5) for elm in list(ds['parameters_lon'][0].values)] ,[round(elm, 5) for elm in list(ds['parameters_lat'][0].values)]
parameter = list((ds['soil_ground_water_level'] * weights).sum('date').values[0]/(len(incl_months) ))
...
weights = re_calculate_season.month_length.groupby('date.month') / re_calculate_season.month_length.astype(float).groupby('date.month').sum()

# Test that the sum of the weights for each season is 1.0
np.testing.assert_allclose(weights.groupby('date.month').sum().values, np.ones(4))

ds= re_calculate_season.ds.assign(mean_summer_sgwl = (re_calculate_season.ds['soil_ground_water_level'] * weights).groupby('date.month').sum('date').sel(month = "6") )
